...
from util.loadSensor import get_data, get_Mydata, get_percent_Mydata, temporal_interpolation, get_Mydata_excludeFeature
    ...
    for featureID in [6,5,4,3,2]:
        ...
        for loo in LOO_list:
            ...
            for subfile in subs[:5]:
                ...
                global max_lr
                train_dataset, valid_dataset, test_dataset = get_Mydata_excludeFeature(subfile, data_path, 0, target_sample=512, featureID=featureID)
                batch_size = 64
                max_epochs = 200
        ...
        print(results_df)
        results_df.to_csv(f'excludeFeature_{featureID}_louo_foundation_results.csv', index=False)
        print(f"Results saved to excludeFeature_{featureID}_louo_foundation_results.csv")
